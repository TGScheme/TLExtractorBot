...
func (ctx *Client) MergeUpstream(rawScheme *types.RawTLScheme, patchOs types.PatchOS, remoteOrder bool, upstream func(isE2E bool) (*types.TLRemoteScheme, error)) (*types.TLFullScheme, error) {
	mergeUpstream := func(rawScheme *types.RawTLScheme, isE2E bool) (*types.RawTLScheme, error) {
		scheme, err := upstream(isE2E)
		if err != nil {
			return nil, err
		}
		return ctx.mergeSchemes(scheme, newFromRaw(rawScheme, isE2E), rawScheme.Layer, patchOs, remoteOrder)
	}
	...
	return &types.TLFullScheme{
		MainApi: ctx.fixDeprecations(mainScheme).TLScheme,
