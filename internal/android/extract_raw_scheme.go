...
import (
	"fmt"
	"github.com/TGScheme/TLExtractorBot/internal/android/astparser"
...
func extractRawScheme(workDir string) (*types.RawTLScheme, error) {
	classes, language, err := java.GetClasses(workDir)
	if err != nil {
		return nil, err
	}

	layer, err := extractLayer(workDir)
	if err != nil {
		return nil, err
	}
	...
	for _, parent := range slices.Sorted(maps.Keys(classes)) {
		...
		for _, className := range slices.Sorted(maps.Keys(parentClasses)) {
			...
			parsed, err := astparser.Parse(astClass, language)
			if err != nil {
			...
			mtProtoName, methLayer := astparser.FixTypeName(className, hint, false)
			...
			if parsed.IsMethod {
				...
				if bareType {
					baseScheme.Type, _ = astparser.FixTypeName(className, "", true)
				} else {
					baseScheme.Type, _ = astparser.FixTypeName(astClass.ExtendsName, astClass.ExtendsPackage, false)
				}
				if baseScheme.Type == "" {
					continue
				}
